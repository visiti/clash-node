...
export function readNodes(dirPath = 'src/nodes') {
    try {
        ...
        files.forEach(file => {
            let clashNodes = YAML.parse(fs.readFileSync(file, 'utf-8')).proxies;
...
export function writeNode(bolb, path = './node-yaml.yaml', templatePath = 'src/config/template.yaml') {
    try {
        let template = YAML.parse(fs.readFileSync(templatePath, 'utf-8'));
        template.proxie= bolb;
